...
resource "yandex_vpc_security_group" "balancer" {  
  ...
  description =  "alb из сети Инертнет по HHTP (80)" 
  network_id = yandex_vpc_network.network-vm.id 
  ingress {
    protocol = "ANY" 
    description = "Health"
...
resource "yandex_alb_http_router" "albhttproute" {
  name          = "albhttprt"
}
resource "yandex_alb_virtual_host" "albvtht" {
  name                    = "albvtht"
  http_router_id          = yandex_alb_http_router.albhttproute.id
  route {
    name                  = "route"
    http_route {
      http_match {
